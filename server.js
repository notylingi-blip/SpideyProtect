...
function isAdmin(req, res, next) {
  if (!req.session || !req.session.user || req.session.user.id !== ADMIN_USER_ID) return res.status(403).send("Forbidden");
  next();
}
...
app.get("/api/scripts", requireAuth, (req, res) => {
  ...
  res.json(db.filter((s) => s.ownerId === userId).map((s) => ({
    id: s.id, name: s.name, enabled: s.enabled, createdAt: s.createdAt,
  })));
});
...
app.get("/api/scripts/internal", (req, res) => {
  ...
  res.json(filtered.map((s) => ({
    id: s.id, name: s.name, enabled: s.enabled, ownerId: s.ownerId,
    ownerUsername: s.ownerUsername, guildId: s.guildId,
  })));
});
...
app.get("/api/scripts/internal/guild/:guildId", (req, res) => {
  if (req.headers["x-api-secret"] !== API_SECRET) return res.status(403).json({ error: "Forbidden" });
  const db = readDB();
  const filtered = db.filter((s) => s.guildId === req.params.guildId);
  res.json(filtered.map((s) => ({
    id: s.id, name: s.name, enabled: s.enabled, ownerId: s.ownerId,
    ownerUsername: s.ownerUsername, guildId: s.guildId,
  })));
...
app.post("/api/scripts", requireAuth, (req, res) => {
  ...
  res.json({
    ...
    loader: `${base}/files/loaders/${id}.lua`,
...
app.get("/api/execute/:id", (req, res) => {
  ...
  res.status(200).type("text/plain").set("Cache-Control", "no-store").send(source);
});
...
app.get("/api/loader/:id.lua", (req, res) => {
  ...
  if (!script) {
    return res.status(404).type("text/plain").send("-- SpideyProtect: Script not found");
  }
  ...
  // ── Free mode request dari executor ──
  if (req.query.freemode === "1") {
    if (!isFreeMode) {
  ...
  if (key) {
    ...
    if (keyData.hwid) {
      ...
      if (idx !== -1) { allKeys[idx].hwid = clientHwid; writeKeys(allKeys); }
...
body{min-height:100vh;font-family:'Segoe UI',Arial,sans-serif;
  background:radial-gradient(circle at 10% 0%,rgba(255,0,0,.25),transparent 35%),
             radial-gradient(circle at 90% 100%,rgba(0,110,255,.20),transparent 35%),#050505;
  display:flex;align-items:center;justify-content:center;padding:20px}
.card{max-width:650px;width:100%;padding:35px 30px;border-radius:20px;
  border:1px solid rgba(90,150,255,.18);
  background:linear-gradient(145deg,rgba(8,31,57,.92),rgba(4,15,28,.95));
  box-shadow:0 25px 70px rgba(0,0,0,.6);text-align:center}
...
.subtitle{color:rgba(255,255,255,.5);font-size:12px;margin-bottom:20px}
...
.free-badge{display:inline-block;background:#00c853;color:white;
  padding:2px 12px;border-radius:12px;font-size:11px;font-weight:800;margin-left:6px}
...
.code-block{width:100%;background:#02060c;border-radius:12px;
  border:1px solid rgba(255,255,255,.08);padding:16px 18px;
...
.copy-btn{width:100%;margin-top:12px;padding:13px;border:none;border-radius:11px;
  ...
  background:linear-gradient(90deg,#d40000,#006eff);transition:transform .2s,filter .2s}
...
.footer-text{margin-top:16px;font-size:11px;color:rgba(255,255,255,.25)}
...
<div class="card">
  ...
  <div class="security-note">
    ...
    ${isFreeMode
      ...
      : '🔑 <strong>Key Required</strong> — Replace YOUR_KEY_HERE with a valid key.'}
  </div>
...
app.get("/api/admin/scripts", isAdmin, (req, res) => {
  ...
  res.json(db.map((script) => {
    const fp = path.join(SCRIPTS_DIR, script.filename);
    return { ...script, source: fs.existsSync(fp) ? fs.readFileSync(fp, "utf8") : null };
  }));
});
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{min-height:100vh;font-family:Arial,sans-serif;background:#050505;color:white;display:flex;align-items:center;justify-content:center}.card{max-width:600px;width:100%;padding:40px;border-radius:20px;border:1px solid rgba(90,150,255,.18);background:linear-gradient(145deg,rgba(8,31,57,.92),rgba(4,15,28,.95));box-shadow:0 25px 70px rgba(0,0,0,.4)}h1{margin-bottom:20px;text-align:center}.stat{display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid rgba(255,255,255,.08)}.stat:last-child{border-bottom:none}.label{color:rgba(255,255,255,.55)}.value{font-weight:bold}.back{display:inline-block;margin-top:25px;padding:10px 20px;border-radius:8px;background:#5865F2;color:white;text-decoration:none;font-weight:bold}</style>
</head>
<body><div class="card">
  ...
  <div style="text-align:center"><a class="back" href="/">⬅ Back</a></div>
</div></body></html>`);
});
...
app.get("/admin/source", isAdmin, (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Source Scripts - Admin</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{min-height:100vh;font-family:Arial,sans-serif;background:#050505;color:white;padding:20px}.container{max-width:1200px;margin:0 auto}h1{margin-bottom:20px;display:flex;align-items:center;gap:15px;flex-wrap:wrap}.header-actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}table{width:100%;border-collapse:collapse;background:rgba(255,255,255,.04);border-radius:12px;overflow:hidden}th,td{padding:12px 15px;text-align:left;border-bottom:1px solid rgba(255,255,255,.08)}th{background:rgba(255,255,255,.08);color:#aaa;font-size:13px;text-transform:uppercase}td{font-size:14px}.script-name{font-weight:bold;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.owner{color:#aaa;font-size:13px;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.status{font-size:12px;padding:4px 8px;border-radius:12px}.status.enabled{background:#1a5a2a;color:#88ff88}.status.disabled{background:#5a1a1a;color:#ff8888}.actions{display:flex;gap:6px;flex-wrap:wrap}.actions button{padding:6px 12px;border:none;border-radius:6px;cursor:pointer;font-weight:bold;font-size:12px;transition:transform .15s}.actions button:hover{transform:scale(1.05)}.btn-view{background:#5865F2;color:white}.btn-copy{background:#2b7a3a;color:white}.btn-download{background:#a9701a;color:white}.modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);align-items:center;justify-content:center;z-index:999}.modal-content{max-width:900px;width:92%;max-height:85vh;background:#111;border-radius:16px;padding:25px;overflow-y:auto;border:1px solid rgba(255,255,255,.1)}.modal-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;flex-wrap:wrap;gap:10px}.modal-header h2{margin:0}.modal-actions{display:flex;gap:8px;flex-wrap:wrap}.modal-actions button{padding:8px 16px;border:none;border-radius:6px;cursor:pointer;font-weight:bold;transition:transform .15s}.modal-actions button:hover{transform:scale(1.05)}.modal-actions .btn-close{background:#444;color:white}.modal-actions .btn-copy-source{background:#2b7a3a;color:white}.modal-actions .btn-download-source{background:#a9701a;color:white}.modal-content pre{background:#0a0a0a;padding:16px;border-radius:8px;overflow-x:auto;font-size:13px;white-space:pre-wrap;word-wrap:break-word;max-height:60vh;overflow-y:auto}.back{display:inline-block;margin:20px 0;padding:8px 16px;background:#5865F2;color:white;text-decoration:none;border-radius:6px}.search-box{padding:10px 16px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:#101010;color:white;width:250px;font-size:14px}.search-box::placeholder{color:#666}.table-wrap{overflow-x:auto}</style>
</head>
<body>
<div class="container">
  ...
  <div class="table-wrap"><div id="scriptsContainer"><p>Loading...</p></div></div>
</div>
...
async function loadScripts(){
  ...
  try{const r=await fetch('/api/admin/scripts');allScripts=await r.json();renderScripts(allScripts);}
  catch(e){c.innerHTML='<p>Error loading scripts.</p>';}
}
function renderScripts(data){
  const c=document.getElementById('scriptsContainer');
  if(!data.length){c.innerHTML='<p>No scripts found.</p>';return;}
  let h=\`<table><thead><tr><th>Name</th><th>Owner</th><th>Status</th><th>Created</th><th>Actions</th></tr></thead><tbody>\`;
  data.forEach(s=>{
    const sc=s.enabled?'enabled':'disabled',st=s.enabled?'Enabled':'Disabled';
    const cr=s.createdAt?new Date(s.createdAt).toLocaleDateString():'Unknown';
    h+=\`<tr><td class="script-name" title="\${escapeHtml(s.name)}">\${escapeHtml(s.name)}</td>
    <td class="owner">\${escapeHtml(s.ownerUsername||s.ownerId)}</td>
    <td><span class="status \${sc}">\${st}</span></td><td>\${cr}</td>
    <td class="actions">
...
async function getScript(id){const r=await fetch('/api/admin/scripts');const d=await r.json();return d.find(s=>s.id===id);}
async function viewSource(id){
  try{const s=await getScript(id);if(!s){alert('Not found');return;}currentScript=s;
  document.getElementById('modalScriptName').textContent=s.name;
  document.getElementById('modalOwner').textContent=s.ownerUsername||s.ownerId;
  document.getElementById('modalScriptId').textContent=s.id;
  document.getElementById('modalSource').textContent=s.source||'-- Source not available';
  document.getElementById('sourceModal').style.display='flex';}catch(e){alert('Error');}
}
async function copyScriptSource(id){try{const s=await getScript(id);if(!s)return;await navigator.clipboard.writeText(s.source||'');alert('✅ Copied!');}catch(e){alert('❌ Failed');}}
async function downloadScriptSource(id){
  try{const s=await getScript(id);if(!s)return;
  const blob=new Blob([s.source||''],{type:'text/plain'});const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download=\`\${s.name.replace(/[^a-zA-Z0-9]/g,'_')}.lua\`;
  document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);}catch(e){alert('❌ Failed');}
}
function closeModal(){document.getElementById('sourceModal').style.display='none';}
async function copySource(){if(!currentScript)return;try{await navigator.clipboard.writeText(currentScript.source||'');alert('✅ Copied!');}catch{alert('❌ Failed');}}
async function downloadSource(){
  if(!currentScript)return;
  const blob=new Blob([currentScript.source||''],{type:'text/plain'});const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download=\`\${currentScript.name.replace(/[^a-zA-Z0-9]/g,'_')}.lua\`;
...
app.get("/admin/bot-servers", isAdmin, (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Bot Servers - Admin</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{min-height:100vh;font-family:Arial,sans-serif;background:#050505;color:white;padding:20px}.container{max-width:800px;margin:0 auto}h1{margin-bottom:20px}.guild-list{list-style:none;padding:0}.guild-item{background:rgba(255,255,255,.05);border-radius:12px;padding:16px 20px;margin-bottom:10px;display:flex;align-items:center;gap:15px;border:1px solid rgba(255,255,255,.06)}.guild-icon{width:50px;height:50px;border-radius:50%;background:#2c2f33;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:bold;color:#aaa;flex-shrink:0}.guild-icon img{width:100%;height:100%;border-radius:50%;object-fit:cover}.guild-info{flex:1}.guild-name{font-weight:bold;font-size:18px}.guild-id{color:#888;font-size:13px}.guild-members{color:#666;font-size:13px}.back{display:inline-block;margin:20px 0;padding:8px 16px;background:#5865F2;color:white;text-decoration:none;border-radius:6px}.no-guilds{padding:40px;text-align:center;color:#666;border:1px dashed rgba(255,255,255,.12);border-radius:12px}</style>
</head>
...
async function loadGuilds(){
  ...
  try{const r=await fetch('/api/admin/guilds');const data=await r.json();
  if(!data.length){c.innerHTML='<div class="no-guilds">🤖 Bot is not in any guild yet.</div>';return;}
  let h='<ul class="guild-list">';
  data.forEach(g=>{
    const ic=g.icon?\`<img src="https://cdn.discordapp.com/icons/\${g.id}/\${g.icon}.png" alt="">\`:escapeHtml((g.name||'?').charAt(0).toUpperCase());
    h+=\`<li class="guild-item"><div class="guild-icon">\${ic}</div><div class="guild-info">
  ...
  h+='</ul>';c.innerHTML=h;}catch(e){c.innerHTML='<p>Error loading guilds.</p>';}
}
function escapeHtml(str){if(!str)return'';return str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
loadGuilds();
</script></body></html>`);
...
@media(max-width:700px){
  .header{padding:18px}.user-name{display:none}.container{width:calc(100% - 20px);margin-top:20px}
  .hero{padding:20px}.form-grid{grid-template-columns:1fr}.file-row{flex-direction:column;align-items:stretch}
  textarea{grid-column:auto}.upload-button{grid-column:auto}
}
</style>
</head>
<body>
...
fileInput.addEventListener("change",function(){
  ...
  scriptName.value=f.name.replace(/\\.(lua|txt)$/i,"");
  const r=new FileReader();
  r.onload=function(e){scriptSource.value=e.target.result;};
  r.readAsText(f);
});
...
document.addEventListener("click",function(e){
  ...
  if(m&&!e.target.closest(".admin-menu"))m.classList.remove("show");
});

async function uploadScript(){
  const name=scriptName.value.trim();const source=scriptSource.value;
